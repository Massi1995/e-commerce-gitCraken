...
class LyraOney34Payment extends AbstractLyraPayment
{
    protected $prefix = 'LYRA_ONEY34_';
    ...
    public function isAvailable($cart)
    {
        ...
        if (! LyraTools::checkOneyRequirements($cart, $this->label)) {
            return false;
        }
    ...
    public function validate($cart, $data = array())
    {
        ...
        $errors = LyraTools::checkAddress($billing_address, 'billing', 'oney34');

        if (empty($errors)) {
            // Billing address is valid, check delivery address.
            $delivery_address = new Address((int) $cart->id_address_delivery);

            $errors = LyraTools::checkAddress($delivery_address, 'delivery', 'oney34');
        }
    ...
    public static function getAvailableOptions($cart)
    {
        ...
        $options = @unserialize(Configuration::get('LYRA_ONEY34_OPTIONS'));
    ...
    public function getTplVars($cart)
    {
        ...
        $vars['lyra_oney_options'] = self::getAvailableOptions($cart);
