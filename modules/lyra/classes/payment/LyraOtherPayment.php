...
class LyraOtherPayment extends AbstractLyraPayment
{
    protected $prefix = 'LYRA_OTHER_';
    ...
    public function getTplVars($cart)
    {
        ...
        $vars['lyra_other_payment_code'] = $this->payment_code;
        $vars['lyra_other_payment_label'] = $cards[$this->payment_code];
    ...
    public function getPaymentOption($cart)
    {
        ...
        $inputs[] = array('type' => 'hidden', 'name' => 'lyra_payment_code', 'value' => $this->payment_code);
        $inputs[] = array('type' => 'hidden', 'name' => 'lyra_payment_title', 'value' => $this->getTitle((int) $cart->id_lang));
    ...
    {
        // Other payment means.
        $other_payment_means = @unserialize(Configuration::get('LYRA_OTHER_PAYMENT_MEANS'));
