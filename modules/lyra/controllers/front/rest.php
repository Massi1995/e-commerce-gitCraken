...
class LyraRestModuleFrontController extends ModuleFrontController
{
    public $ssl = true;
    ...
    public function postProcess()
    {
        ...
        $sha_key = $test_mode ? Configuration::get('LYRA_RETKEY_TEST') : Configuration::get('LYRA_RETKEY_PROD');

        // Use direct post content to avoid stipslashes from json data.
        $data = $_POST;

        // Check the authenticity of the request.
        if (! LyraTools::checkHash($data, $sha_key)) {
            $ip = Tools::getRemoteAddr();
            $this->logger->logError("{$ip} tries to access module/lyra/rest page without valid signature with parameters: " . print_r($data, true));
            ...
            if ($response->isAcceptedPayment()) {
                ...
                if ($save_on_failure) {
                    ...
                    if (! $response->isCancelledPayment()) {
                        ...
                        $this->context->cookie->lyraPayErrors = $this->module->l('Your payment was not accepted. Please, try to re-order.', 'submit');
                    }
