...
class OrderHistoryCore extends ObjectModel
{
    ...
    {
        /**************** Test la Connexion a L'API ************************/
        if (!parent::add($autodate)) {
            return false;
        }
        $order = new Order((int)$this->id_order);
        // Update id_order_state attribute in Order
        if ($this->id_order_state == 20) {
            #ENVOI API
            include 'override/controllers/Api.php';
            $api = new Api();
            $resultApi = $api->postOrderToApi($order);
            //DANS LE CAS OU LE RETOUR DE API EST POSITIVE
            if ($resultApi['codeRetour'] == 200)
            {
                //METTRE A ETAT RNVOYEE SUR BEXT
                $this->id_order_state = 20;

            }
            /*else if ($resultApi['codeRetour']=='')
             {
                 //RETOUR NUL DE LAPI
                $this->id_order_state = 23;
            }*/
            else if ($resultApi['codeRetour']==400)
            {
                if ($resultApi['msgRetour']=="Order already exists")
                {
                    //COMMANDE EXISTANTE DANS BEXT
                    $this->id_order_state = 22;
                }else
                {
                    //ECHEC API
                    $this->id_order_state = 21;
                }

                #Données a envoyée en cas d'erreur
                $numCommande = $resultApi['numCommande'];

                #Le sujet du mail
                $objetmail = "Echec de descente de la commande $numCommande  vers Bext";

                #le contenu du mail
                $jsonCommande = $resultApi['jsonOrder'];
                $jsonRetour = $resultApi['msgRetour'];
                $message = "
                                   <html>
                                    <head>
                                         <title></title>
                                        ...
                                        <body>                                     
                                           <p><h1>Le message retour de l'api</h1></p>
                                           <pre> " . $jsonRetour . "</pre>
                                           <p><h1>Les informations de la commande</h1> </p>
                                            <pre>" . $jsonCommande . "</pre>
                ...
                mail('supportvap@cmp-paris.com', $objetmail, $message, implode("\r\n", $headers));
